...
(function ($) {
	$.fn.barousel = function (callerSettings) {
		var settings = $.extend({
			...
			extraNavWrapper: '.barousel_extra_nav',
		...
		settings.extraNavWrapper = $(this).find(settings.extraNavWrapper);
		...
		if (settings.totalItem == 1) {
			...
			if (settings.navType == 1) {
				...
				settings.imageList.each(function (n) {
					...
					strNavList += "<li><a href='#' title='" + $(settings.contentList[n]).find('p.header').text() + "' class='" + currentClass + "'>" + (n + 1) +"<img src='images/down_arrow.png' /></a></li>";
				});
				strNavList += "</ul>";
				...
				//set the index of each nav link
				settings.navList.each(function (n) { this.index = n; });
			} else if (settings.navType == 4) {
				//items navigation type
				var strNavList = "<ul>";
				settings.imageList.each(function (n) {
					var currentClass = "";
					if (n == 0) currentClass = "current";
					strNavList += "<li><a href='#' title='" + $(settings.contentList[n]).find('p.header').text() + "' class='" + currentClass + "'>" + (n + 1) +"<img class='arrow' src='images/down_arrow.png' alt='' /></a></li>";
				});
				strNavList += "</ul>";
				settings.navWrapper.append(strNavList);
				settings.navList = settings.navWrapper.find('a'); //list of the items' nav link
				...
				//prev/next navigation type
				var strNavList = "<div class='prev'><a href='#' title='previous'></a></div>";
				strNavList += "<div class='next'><a href='#' title='next'></a></div>";
				strNavList += "<div class='counter'><span class='counter_current'>1</span>/<span class='counter_total'>" + settings.totalItem + "</span></div>";
				settings.extraNavWrapper.append(strNavList);
				settings.extraNavList = settings.extraNavWrapper.find('a'); //list of the items' nav link
			}
			...
			if (settings.navType == 1 || settings.navType == 3) {
				...
				//prev/next navigation type
				settings.extraNavList.each(function () {
					$(this).click(function () {
						if (settings.navFreeze == 0) {
							window.clearTimeout(settings.timerCarousel);
							settings.stopCarousel = 1;
	...
	var loadModuleContent = function (settings, index) {
		...
		if (settings.navType == 4) {
			$(settings.extraNavWrapper).find('.counter_current').text(index + 1);
		}
	};
